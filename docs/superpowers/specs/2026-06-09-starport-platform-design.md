...
> 设计文档 · 2026-06-09 · 状态：**v2**（按用户反馈修订）
> 工作代号：星港 / StarPort（仅作品牌名使用，产品内不再使用航海/星舰隐喻文案）

## v2 修订要点（用户反馈，优先级最高）

1. **应用发布模型**：平台上所有应用（含 multillm-chat 等自家应用）都**不写在平台仓库里**，而是独立开发部署、经开发者中心发布到平台。平台仅承载 listing（详情页/版本/评测）+ 运行时（沙箱 iframe/webview 加载应用 URL + Platform SDK 注入）。§3.2 / §4.4 据此理解；monorepo 集成方案作废。
2. **视觉改向**：废弃 v1 深空暗色风。新方向：**明亮、简洁、低装饰** —— 浅灰底、白色卡片、单一蓝色强调色、细边框、克制的阴影与动效；禁用辉光/玻璃拟态/渐变字/全大写宽字距标签等"AI 味"元素。信息密度与版式继续对标 Steam。
3. **去隐喻命名**：UI 文案用直白词。映射：港湾→**库**、星籍主页→**个人主页**、星潮→**动态**、港务局→**开发者中心**、造物→**产品/应用**、星尘→**点数**。本文 v1 章节中的旧称按此映射阅读。
4. **全局好友面板**：好友不藏在个人主页。仿 Steam"好友与聊天"——全局右下角常驻入口，展开成好友列表（在线状态/正在使用什么），可直接发消息。Phase 0.5 先做前端壳 + 假数据聊天，Phase 2 接真实消息。
5. **桌面化目标**：Web 是过渡形态，最终交付桌面 App。新增 **Phase 7：桌面化**（Tauri/Electron 壳加载站点、系统托盘、好友消息通知、应用以独立窗口运行）。架构上避免依赖"仅浏览器标签页"假设（如好友面板做成独立可拆窗口的组件）。
