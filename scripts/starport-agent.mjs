...
let syncFilesOn = false;
let pendingAck = []; // 上一轮已写回本机的路径，下次同步时 ack
const SYNC_MAX_FILES = 50;
const SYNC_MAX_BYTES = 180_000;
...
/** 收集工作目录里要同步给平台的文件（上下文文件 + memory/ 下的文本） */
function gatherWorkdirFiles() {
  const ctx = BACKEND === "gemini" ? "GEMINI.md" : BACKEND === "qwen" ? "QWEN.md" : "CLAUDE.md";
  const out = [];
  const tryPush = (rel) => {
    if (out.length >= SYNC_MAX_FILES) return;
    try {
      const full = join(DIR, rel);
      if (existsSync(full) && statSync(full).isFile()) {
        let content = readFileSync(full, "utf8");
        if (content.length > SYNC_MAX_BYTES) content = content.slice(0, SYNC_MAX_BYTES);
  ...
  try {
    ...
    if (existsSync(memDir) && statSync(memDir).isDirectory()) {
      for (const name of readdirSync(memDir)) {
        if (/\.(md|txt|json)$/i.test(name)) tryPush("memory/" + name);
      }
    }
  } catch {
    /* 无 memory 目录 */
...
/** 一轮文件同步：上传本机内容 + ack 已写回；把平台的待写回编辑落到本机磁盘 */
async function syncWorkdirFiles() {
  try {
    const files = gatherWorkdirFiles();
    const res = await api("/api/v1/agent/files", { method: "POST", body: JSON.stringify({ files, acked: pendingAck }) });
    pendingAck = [];
...
const main = async () => {
  ...
  if (syncFilesOn) {
    log("文件同步已开启：上传工作目录文件到平台");
    await syncWorkdirFiles();
  }
  let backoff = 2000;
  // 先处理首拉就带回来的任务
  ...
  for (;;) {
    ...
    try {
      ...
      if (res.agent && res.agent.syncFiles !== undefined) syncFilesOn = !!res.agent.syncFiles;
      if (syncFilesOn) await syncWorkdirFiles();
      pending = res.tasks ?? [];
      backoff = 2000;
    } catch (e) {
